...
from mmdet.models.utils import Dynamic_conv2d


@HEADS.register_module()
class CPointsFGDDCHead(AnchorFreeHead):
    """RepPoint head.
    ...
    def _init_layers(self):
        ...
        self.reppoints_pts_init_dy_conv = Dynamic_conv2d(self.feat_channels, 
                                                         self.point_feat_channels, 3, padding=1, stride=1, 
                                                         groups=18)
    ...
    def forward_single(self, x):
        ...
        for init_conv in self.init_convs:
            x = init_conv(x)
        cls_feat = x
        pts_feat = x
        # initialize reppoints
        pts_out_init = self.reppoints_pts_init_out(
            self.relu(self.reppoints_pts_init_dy_conv(pts_feat)))
